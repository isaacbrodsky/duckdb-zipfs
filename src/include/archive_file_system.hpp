...
//
// These entries are NOT randomly seekable (CanSeek() == false): a compressed
// stream cannot be seeked, so DuckDB reads them forward. Forward reads
// decompress-and-discard to reach the position; a backward position throws
// rather than re-inflating from the start. Sequential forward reads (the common
// CSV/NDJSON case) are O(n) with O(BLOCK_SIZE) memory.
...
public:
  ...
  // Compressed archive entries cannot be randomly seeked.
